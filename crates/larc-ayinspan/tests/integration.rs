...
use larc_ayinspan::batch::{BatchConfig, SpanBatcher};
use larc_ayinspan::emit::SpanEmit;
...
async fn backpressure_overflow_increments_dropped_counter() {
    ...
    larc_ayinspan::metrics::reset_for_test();
    ...
    let initial = larc_ayinspan::metrics::spans_dropped_total();
    ...
    let dropped = larc_ayinspan::metrics::spans_dropped_total() - initial;
    assert!(
        dropped >= 8,
