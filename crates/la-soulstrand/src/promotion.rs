...
use crate::{SoulstrandError, Step, Tier};
...
pub trait PromotionBackend: Debug + Send + Sync {
    ...
    async fn promote(&self, steps: Vec<Step>) -> Result<Vec<Step>, SoulstrandError>;
    ...
    async fn tier_step(&self, id: &str, tier: Tier) -> Result<(), SoulstrandError>;
    ...
    async fn deduplicate(&self, steps: Vec<Step>) -> Result<Vec<Step>, SoulstrandError> {
        Ok(steps)
    }
}
